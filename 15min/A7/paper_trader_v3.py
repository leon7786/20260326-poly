...
class PaperTraderV3:
    ...
    def _save_state(self):
        ...
        balance = self.initial_balance + self.total_pnl
    ...
    def _do_settle(self, key, w, coin, close_price):
        ...
        self._log(
            ...
            f"| 余额${(self.initial_balance + self.total_pnl):.2f} (PnL ${self.total_pnl:+.2f}) WR={wr:.0f}% ({self.total_count}笔)"
        )
    ...
    def _check_and_trade(self, coin):
        ...
        # 改进 B: 动量阈值微调
        mom_threshold = 0.015 if coin.lower() in ["btc", "eth"] else 0.02
        ...
        if self.daily_trades >= self.cfg.max_daily_trades or self.daily_pnl <= self.cfg.max_daily_loss:
            return
        if (self.initial_balance + self.total_pnl) < self.cfg.bet_size:
            self._log(f"⚠️ 余额不足，无法下单。当前余额 ${(self.initial_balance + self.total_pnl):.2f}")
            return

        market = self._get_market(coin, start_unix)
        buy_price, price_source, book_info = self._resolve_buy_price(market, direction)

        # 改进 A & C: 动态入场价格上限 & 极高确定性扫单
        dynamic_max_buy = self.cfg.max_buy_price
        is_sweep = False
        if win_prob >= 0.95 and secs_left <= 180:
            dynamic_max_buy = 0.85
            if abs(momentum) >= 0.10 and consensus_ratio == 1.0:
                is_sweep = True
                
        # TODO: 仓位限制（目前简单实现：已交易过就不再交易，后续可以加入补仓逻辑，这里先保持原有的 traded 限制）
        # 暂不修改 w["traded"] 的限制，先应用动态买入价和日志标识
        ...
        if self.cfg.strict_real_odds and price_source == "none":
            ...
            self.total_skips += 1
            return
        if buy_price is not None and buy_price > dynamic_max_buy:
            sk = f"{window_key}-{coin}-expensive"
            if sk not in self._logged_skips:
                self._logged_skips.add(sk)
                self._log(f"⏭ {coin.upper()} {direction} | ask=${buy_price:.2f}({price_source}) > ${dynamic_max_buy:.2f} | book={book_info.get('bid_levels', 0)}b/{book_info.get('ask_levels', 0)}a")
            self.total_skips += 1
            return
        ...
        sweep_tag = "🚀[扫单]" if is_sweep else "🎯"
        self._log(
            f"{sweep_tag} {coin.upper()} {direction} | prob={win_prob:.0%} mom={momentum:+.3f}% "
    ...
    def run(self):
        ...
        print(f"  资金: 初始 ${self.initial_balance:.2f} | 余额 ${(self.initial_balance + self.total_pnl):.2f} (PnL ${self.total_pnl:+.2f}) | {self.total_count}笔", flush=True)
